...
export default function App() {
  ...
  async function doBulkAdd() {
    ...
    setBulkLoading(true);setBulkResults([]);setBulkMsg("");setBulkError("");
...
Return ONLY a valid JSON array with no markdown fences, no preamble, no explanation. Each item must have exactly these fields:
      ...
      arr.forEach(p=>{
        if(p.name&&!p.dnh_flag){
          ...
          updateCands(cs=>[...cs,{
            ...
            name:p.name,role:bulkRole,company:p.company||"",title:p.title||"",
            score:p.score||0,stage:"Sourced",outreach:"",followUp:"",response:"",
            signals:p.signals||[],notes:p.summary||"",location:p.location||"",
            relocation:false,source:"LinkedIn",linkedin:"",
            park:p.park||false,parkReason:p.park_reason||"",parkDate:"",parkNote:p.park_reason||""
          }]);
...
DO NOT HIRE if currently at: ${DNH.join(", ")}. Flag explicitly if detected.
RUBRIC: ${role.rubric.map(r=>`${r.l} (${r.w}pts)`).join(", ")}
SIGNALS TO LOOK FOR: ${role.signals.join(", ")}
CANDIDATE PROFILE:
${scoreInput}
...
{"name":"","score":0,"recommendation":"Strong Yes/Yes/Maybe/No","dnh_flag":false,"dnh_reason":"","summary":"","signals_matched":[],"rubric_scores":[{"label":"","score":0,"max":0}],"pros":[],"cons":[],"outreach_worthy":true,"park_suggestion":false,"park_reason":""}`;
    try {
      ...
      if(parsed.name&&parsed.score&&!parsed.dnh_flag){
        updateCands(cs=>[...cs,{
          id:Date.now(),name:parsed.name,role:scoreRole,company:"",title:"",
          score:parsed.score,stage:"Sourced",outreach:"",followUp:"",response:"",
          signals:parsed.signals_matched||[],notes:parsed.summary||"",location:"",
          relocation:false,source:"LinkedIn",linkedin:"",
          park:parsed.park_suggestion||false,parkReason:parsed.park_reason||"",parkDate:"",parkNote:parsed.park_reason||""
        }]);
      }
    } catch(e) {
  ...
  async function genMsg(cand, type) {
    setMsgCand(cand);setMsgType(type);setMsgLoading(true);setMsgText("");setMsgError("");
    const role=getRoleById(cand.role);
    const isInmail=type==="inmail";
    const prompt=isInmail
      ?`You are Shawn Johnsen, Sr. Sourcer at Crusoe Energy (internal, not agency). Write a LinkedIn InMail for this candidate.

STRICT RULES:
- Length: 1,600–1,800 characters total
- Open by introducing yourself as Shawn Johnsen, Sr. Sourcer at Crusoe Energy (internal, not agency)
- Sell Crusoe and the role FIRST before explaining why you are reaching out
- Explain specifically why you are reaching out based on their background
- Include 2–3 smart clarifying questions relevant to the role
- Include the job link: ${role?.link||"[JD LINK]"}
- End with your email sjohnsen@crusoe.ai and ask for resume or application
- If candidate is not in Denver/Arvada area: note the role is on-site in Arvada (Denver area) with relocation assistance available
- NEVER mention names of other Crusoe employees
- NEVER include a phone number
- NEVER ask for a 15-minute call

ABOUT CRUSOE: ${CRUSOE_BIO}
ROLE: ${role?.title} | JD: ${role?.jd||""}
CANDIDATE: ${cand.name}, ${cand.title||"[title]"} at ${cand.company||"[company]"} (${cand.location||"location unknown"})
MATCHED SIGNALS: ${cand.signals?.join(", ")||"none noted"}

Write only the InMail message text, nothing else:`
      :`You are Shawn Johnsen, Sr. Sourcer at Crusoe Energy. Write a ${FOLLOW_UP_DAYS}-day follow-up LinkedIn message to ${cand.name} who has not responded to your initial outreach.

STRICT RULES:
- Length: 800–1,200 characters total
- Brief friendly re-introduction
- ONE compelling reason about Crusoe or the role to create urgency
- Low pressure, respectful tone
- CTA: share resume or apply at ${role?.link||"[JD LINK]"}
- Include sjohnsen@crusoe.ai
- NEVER mention other Crusoe employee names
- NEVER include a phone number
- NEVER ask for a 15-minute call

ROLE: ${role?.title}
CANDIDATE: ${cand.name}, ${cand.title||"[title]"} at ${cand.company||"[company]"}

Write only the follow-up message text, nothing else:`;
    try {
      const text = await callClaude(prompt, 1000);
      setMsgText(text.trim());
    } catch(e) {
      setMsgError(`Generation failed: ${e.message}`);
    }
    setMsgLoading(false);
  }
  ...
  function copyAndMark(cand,type) {
    ...
    setMsgCand(null);setMsgText("");setMsgError("");
  }
    ...
    return(
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
          {[{l:"Total Active",v:active.length,c:"#60a5fa",s:"in pipeline"},{l:"Outreach Sent",v:out,c:"#a78bfa",s:"InMails + follow-ups"},{l:"Submitted",v:sub,c:"#34d399",s:"to hiring managers"},{l:"HM Screen+",v:hmScreen,c:"#fbbf24",s:"past submission"},{l:"Hired",v:hired,c:"#22c55e",s:"offers accepted"}].map(x=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[{l:"Parked",v:parked.length,c:"#f59e0b",s:"talent bank"},{l:"Phone Screens",v:active.filter(c=>["Phone Screen","Submitted","HM Screen","Interviewing","Offer","Hired"].includes(c.stage)).length,c:"#6366f1",s:"screened"},{l:"Interviewing",v:active.filter(c=>["Interviewing","Offer","Hired"].includes(c.stage)).length,c:"#10b981",s:"active interviews"},{l:"Final/Not Hired",v:finalNotHired,c:"#78716c",s:"completed process"}].map(x=>(
          ...
          <div style={S.card()}>
            ...
            {ROLES.map(r=>{
              ...
              return(<div key={r.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #1f2937",cursor:"pointer"}} onClick={()=>{setView("Pipeline");setRoleFilter(r.id);}}>
                ...
                <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:11,color:"#34d399"}}>{adv} adv</span><span style={{fontSize:11,color:"#9ca3af"}}>{rc.length} total</span><span style={S.badge("#22c55e")}>{r.status}</span></div>
              </div>);
            })}
          </div>
          <div style={S.card()}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {showAddC&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200}}>
        <div style={{background:"#111827",border:"1px solid #374151",borderRadius:12,padding:22,width:420,maxWidth:"95vw"}}>
          ...
          <div style={{marginBottom:9}}><label style={S.lbl}>Notes</label><textarea value={newC.notes} onChange={e=>setNewC(v=>({...v,notes:e.target.value}))} rows={2} style={{...S.inp,resize:"vertical"}}/></div>
      ...
      {msgCand&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}}>
        <div style={{background:"#111827",border:"1px solid #374151",borderRadius:12,padding:22,width:580,maxWidth:"95vw",maxHeight:"90vh",overflowY:"auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <button onClick={()=>{setMsgCand(null);setMsgText("");setMsgError("");}} style={S.btn()}>✕</button>
          </div>
          ...
          {msgLoading&&<div style={{textAlign:"center",padding:16,color:"#6b7280"}}>⏳ Generating message...</div>}
          {msgError&&<div style={{background:"#ef444415",border:"1px solid #ef444430",borderRadius:7,padding:"8px 12px",marginBottom:8,fontSize:12,color:"#ef4444"}}>{msgError}</div>}
          {msgText&&<>
            ...
            <button onClick={()=>copyAndMark(msgCand,msgType)} style={{...S.btn("green"),width:"100%"}}>📋 Copy & Mark Sent</button>
          </>}
        </div>
      </div>}
    </div>
  ...
  const renderBulk = () => (
    <div style={{display:"flex",flexDirection:"column",gap:14,maxWidth:800}}>
      ...
      <div style={S.card()}>
        ...
        {bulkMsg&&<div style={{fontSize:12,color:"#22c55e",marginBottom:8,padding:"8px 10px",background:"#22c55e11",borderRadius:6,border:"1px solid #22c55e33"}}>{bulkMsg}</div>}
        {bulkError&&<div style={{fontSize:12,color:"#ef4444",marginBottom:8,padding:"8px 10px",background:"#ef444415",borderRadius:6,border:"1px solid #ef444430"}}>{bulkError}</div>}
        <button onClick={doBulkAdd} disabled={bulkLoading||!bulkText.trim()} style={{...S.btn("primary"),opacity:bulkLoading||!bulkText.trim()?0.5:1}}>
          {bulkLoading?"⏳ Processing... (this may take 10–20 seconds)":"⚡ Process & Add to Pipeline"}
        </button>
      </div>
      {bulkResults.length>0&&(
        <div style={S.card()}>
          ...
          {bulkResults.map((r,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid #1f2937",flexWrap:"wrap",gap:6}}>
              <div>
                ...
                <div style={{fontSize:11,color:"#6b7280"}}>{r.title}{r.company?` · ${r.company}`:""}</div>
          ...
          <button onClick={()=>{setBulkText("");setBulkResults([]);setBulkMsg("");setBulkError("");}} style={{...S.btn(),marginTop:8,fontSize:11}}>Clear & Add More</button>
        </div>
      )}
    </div>
  );
  ...
  const renderScore = () => (
    <div style={{display:"flex",flexDirection:"column",gap:12,maxWidth:720}}>
      ...
      <div style={S.card()}>
        ...
        <div style={{marginBottom:10}}><label style={S.lbl}>Paste LinkedIn profile or resume text</label><textarea value={scoreInput} onChange={e=>setScoreInput(e.target.value)} rows={9} style={{...S.inp,resize:"vertical"}} placeholder="Paste candidate's LinkedIn profile text or resume here..."/></div>
        <div style={{fontSize:11,color:"#ef4444",marginBottom:8}}>⛔ Auto-flags DNH companies · ✓ Auto-adds to pipeline if not DNH</div>
        {scoreError&&<div style={{fontSize:12,color:"#ef4444",marginBottom:8,padding:"8px 10px",background:"#ef444415",borderRadius:6,border:"1px solid #ef444430"}}>{scoreError}</div>}
        <button onClick={doScore} disabled={scoring||!scoreInput.trim()} style={{...S.btn("primary"),opacity:scoring||!scoreInput.trim()?0.5:1}}>{scoring?"⏳ Scoring... (10–20 seconds)":"⚡ Score Candidate"}</button>
      </div>
        ...
        {scoreResult.error?<div style={{color:"#ef4444"}}>{scoreResult.error}</div>:(
          <>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              ...
              <div style={{textAlign:"center"}}><div style={{fontSize:28,fontWeight:700,color:sc(scoreResult.score)}}>{scoreResult.score}</div><div style={{fontSize:10,color:"#6b7280"}}>/100</div><div style={{marginTop:4}}><span style={S.badge(scoreResult.outreach_worthy?"#22c55e":"#ef4444")}>{scoreResult.recommendation}</span></div></div>
            </div>
            <div style={S.g2}>
              <div><div style={{fontSize:10,color:"#6b7280",marginBottom:5,textTransform:"uppercase",letterSpacing:1}}>Rubric</div>
                {(scoreResult.rubric_scores||[]).map(rb=><div key={rb.label} style={{marginBottom:6}}>
                  ...
                  <div style={{background:"#1f2937",borderRadius:3,height:4}}><div style={{background:"#60a5fa",borderRadius:3,height:4,width:`${rb.max?(rb.score/rb.max)*100:0}%`}}/></div>
                </div>)}
              </div>
              <div>
            ...
            {scoreResult.outreach_worthy&&!scoreResult.dnh_flag&&<div style={{marginTop:8,padding:8,background:"#22c55e11",border:"1px solid #22c55e33",borderRadius:7,fontSize:12,color:"#22c55e"}}>✓ Added to Pipeline automatically. Go to Pipeline to generate InMail.</div>}
          </>
        )}
      </div>}
    </div>
  ...
  const renderOutreach = () => {
    ...
    return(
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={S.card()}>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:8,textTransform:"uppercase",letterSpacing:1}}>Historical (pre-tracker)</div>
          <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
  ...
  // ─── WEEKLY UPDATE ────────────────────────────────────────────────────────────
  const renderWeekly = () => {
    const deptRoles=ROLES.filter(r=>r.category==="Manufacturing");
    const weekAgo=new Date(TODAY);weekAgo.setDate(weekAgo.getDate()-7);
    const thisWeek=candidates.filter(c=>c.outreach&&new Date(c.outreach)>=weekAgo);
    const lines=[
      `MANUFACTURING SOURCING UPDATE — Week of ${TODAY.toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}`,
    ...
    return(
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
          <h1 style={{margin:0,fontSize:18,fontWeight:700}}>Weekly Update</h1>
  ...
  const renderMetrics = () => {
    ...
    return(
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <h2 style={{margin:0,fontSize:18,fontWeight:700}}>📊 Metrics</h2>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[{l:"Response Rate",v:total?Math.round(responded/total*100)+"%":"—",c:"#8b5cf6"},{l:"Screen Rate",v:total?Math.round(screens/total*100)+"%":"—",c:"#6366f1"},{l:"Submit Rate",v:total?Math.round(submitted/total*100)+"%":"—",c:"#10b981"},{l:"HM Screen Rate",v:total?Math.round(hmScreened/total*100)+"%":"—",c:"#f59e0b"}].map(s=>(
        ...
        <div style={S.card()}>
          ...
          {[{l:"Total Sourced",v:total,c:"#475569"},{l:"Responded",v:responded,c:"#8b5cf6"},{l:"Phone Screened",v:screens,c:"#6366f1"},{l:"Submitted",v:submitted,c:"#10b981"},{l:"HM Screen",v:hmScreened,c:"#f59e0b"},{l:"Hired",v:hired,c:"#22c55e"},{l:"Final Stage - Not Hired",v:finalNotHired,c:"#78716c"}].map(({l,v,c})=>(
